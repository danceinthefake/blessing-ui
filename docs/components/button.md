...
Three variants, one job each: **solid** is the action, **outline** the alternative beside it, **ghost** the tertiary thing you'd otherwise make a link. Colour means chosen: every button is ink at rest and fills with the accent while pressed, so the main action stands out by being **solid**, not by colour. Hover fades and leans; focus leans with the accent ring; pressing fills. `color="accent"` is for a button that shows something already chosen (the selected segment of a [ButtonGroup](./button-group)); `danger` for destructive — and if the action can be undone, do it and offer Undo from a [Toaster](./toaster) rather than asking first; `text` (default) for the rest. `type` defaults to `button`, so a button inside a form doesn't submit by accident — say `type="submit"` when it should.
