...
The control for [dark mode](../guide/dark-mode). Default is a single button — "Dark theme", `aria-pressed` when it's on — for a nav bar; `mode="group"` is light / system / dark as a toggle group, for a settings page. The switch can only ever set light or dark, so once pressed it leaves _system_ behind; offer the group where you want the OS to stay in charge.
