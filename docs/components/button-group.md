...
Each button is its own tab stop and carries no selection state. For an exclusive choice with `aria-pressed` — Day / Week / Month where one is _on_ — use [ToggleGroup](./toggle); for a main action with a menu beside it, [SplitButton](./split-button); for several groups with arrow-key roving, [Toolbar](./toolbar).
