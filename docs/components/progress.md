...
The linear one: a task with a known length and room across the page — an upload, a form's steps, a quota. Pass `value` (against `max`, default 100); omit it for an indeterminate bar that slides until you know more. `showValue` prints the percentage at the end of the head, or whatever `format(value, max)` returns. Under `prefers-reduced-motion` the indeterminate bar holds still at 40%.
