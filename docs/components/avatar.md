...
A **petal** — the leaning plate with its two sharp tips softened — because a face is round by nature and a circle would be the one shape here that doesn't lean (the [rule](../guide/conventions#shape-cut-or-petal)). `lean` skews the frame as well, like a plate; the image inside is counter-skewed so the face stays upright.
