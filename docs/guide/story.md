...
- **Breadth.** The site needed a nav, a list and a modal. The library has 145 components, from a button to a data table to a chat thread. Coverage was checked against the component lists of [shadcn/ui](https://ui.shadcn.com/) and [PrimeVue](https://primevue.org/) so nothing obvious was missing; every component here is its own implementation, and nothing from either project is used.
